...
Day 1: 统一 LLM 客户端
========================
核心机制：Delta 流式 SSE 解析、指数退避重试、Token 估算、取消令牌

关键修正（相比 ttt.py v1）：
- Delta 类型消除 _last_tool_calls 副作用（线程安全）
- cancel_event 支持 steering 中断
- 流式和非流式共享重试逻辑
"""
...
async def collect_deltas(stream: AsyncIterator[Delta]) -> AccumulatedResponse:
    ...
    return AccumulatedResponse(text=text, tool_calls=tool_calls)

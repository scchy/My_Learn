...
class CompressionConfig:
    ...
    tier2_ratio: float = 0.70  # 超过 70% → 摘要旧消息
...
class ContextCompressor:
    ...
    async def compress_if_needed(
        ...
        *,
        ...
        if ratio >= self.config.tier3_ratio:
            ...
            return self._tier1_compress(system_msgs, blocks)
    ...
    def _split_into_blocks(self, messages: list[Message]) -> list[list[Message]]:
        ...
        块类型：
        - 单条 user 消息 → 独立块
        - 单条 assistant 消息（无 tool_calls）→ 独立块
        - assistant（含 tool_calls）+ 后续所有 tool 消息 → 合并为一块
        """
    ...
    def _tier1_compress(
        ...
        preserve = self.config.tier1_preserve_blocks
        if len(blocks) <= preserve:
    ...
    async def _tier2_compress(
        ...
        """Tier 2 (70%): 摘要旧块，保留系统提示 + 摘要 + 最近 N 个块。"""
    ...
    async def _tier3_compress(
        ...
        preserve = self.config.tier3_preserve_blocks
        if len(blocks) <= preserve:
            return system_msgs + [m for b in blocks for m in b]

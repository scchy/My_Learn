...
class Agent:
    ...
    async def _stream_and_collect(self) -> "collect_deltas":
        """流式获取 LLM 响应，用 rich.Live 实时渲染文本。"""
        from pi_agent_reference.llm import AccumulatedResponse
        ...
        return AccumulatedResponse(text="".join(text_parts), tool_calls=tool_calls)

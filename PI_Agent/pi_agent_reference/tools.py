...
class ToolRegistry:
    ...
    def _build_schema(self, func: Callable) -> dict[str, Any]:
        """从函数签名构建 JSON Schema。当前只用 str 类型参数简化处理。"""
        import inspect
...
def _read_file(path: str, offset: str = "1", limit: str = "100") -> str:
    """读取文件内容。offset 为起始行号 (1-indexed)，limit 为最大行数。"""
...
def register_builtin_tools(registry: ToolRegistry) -> ToolRegistry:
    ...
    registry.register(_read_file, name="read", description="读取文件内容（支持 offset/limit 分页）")
    registry.register(
        _bash_command,
        name="bash",

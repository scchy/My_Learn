...
void TrtEngine::doInference(int useV2 = 0)
{
  // 将输入数据复制到GPU
    ...
    bool status;
    if (useV2)
        status = context_->executeV2(buffers_->getDeviceBindings().data());
    else
        status = context_->execute(1, buffers_->getDeviceBindings().data());

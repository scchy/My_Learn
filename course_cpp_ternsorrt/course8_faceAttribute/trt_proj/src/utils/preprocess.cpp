...
void facedetPreprocess(const cv::Mat &img, int inputW, int inputH, void *buffer)
{
    ...
    cv::Mat img_float;
    img.convertTo(img_float, CV_32FC3, 1.0 / 255.0); // 将图像转换为float类型，归一化到[0,1]
...
void facenetPreprocess(const cv::Mat &img, int inputW, int inputH, void *buffer)
{
    ...
    std::vector<cv::Mat> nchw_channels;
    cv::split(img_normalized, nchw_channels); // 将输入图像分解成三个单通道图像：rrrrr、ggggg、bbbbb
...
void emotionPreprocess(const cv::Mat &img, int inputW, int inputH, void *buffer)
{
    ...
    cv::Mat img_float;
    img_gray.convertTo(img_float, CV_32FC1, 1 / 255.0);
    cv::Mat img_resized;
    cv::resize(img_float, img_resized, cv::Size(inputW, inputH));

    memcpy(buffer, img_resized.data, inputH * inputW * sizeof(float));
}
...
DEFINE_int32(input_h, 112, "network input height");
DEFINE_int32(input_w, 112, "network input width");
...
int main(int argc, char **argv)
{
    ...
    std::string output_file_name = FLAGS_onnx_file.substr(
        0, FLAGS_onnx_file.find_last_of(".")
    ) + ".engine";
    // 1. Create builder
    ...
    if(!useInt8){
        ...
        auto calibrator = new CalibrationDataReader(
            calib_dir, calib_list_file, input_name, input_h, input_w, preprocess
        );
